  ...
  async joinUser(req: any, res: any) {
    try {
      ...
      if (err.customData._tokenResponse.error.code === ResultCode.BadRequest) {
        res.status(ResultCode.BadRequest).json({
          code: ResultCode.BadRequest,
          message: "이미 존재하는 계정입니다.",
          data: null,
        });
      } else {
        res.status(err.customData._tokenResponse.error.code).json({
    ...
    try {
      if (req.headers["access-token"]) {
        const data = await admin
          .auth()
          .verifyIdToken(req.headers["access-token"]);
        const userInfo: UserInfo = {
          uid: data.uid,
          email: data.email,
