...
import { addData, deleteData, getAllData, getData, setData, updateNumColumn } from '../firebase/firestore';
...
/** 리스트 데이터 추출 */
export const fetchAlcHistory = async (uid: string): Promise<AlcHistoryDaysDrink[]> => {
    const data = await getData(AH_TABLE, uid) as AlcHistoryDaysDrink[];
    return data;
}

export const createAlcHistory = async (uid: string, value: AlcHistoryFormData): Promise<boolean> => {
    try {
        ...
        console.log(newAlcHistory, 123123)
